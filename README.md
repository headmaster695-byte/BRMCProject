...
- Destinations other than First are **thick stubs** (`brmc:dest_climate` cells): False First yellow-mono room, Second crimson-mono P0 room (`brmc:second_*`), Third cold-garage P0 room (`brmc:third_*`), Buttons plant, Second False First soft yellow, Spiral well, Custodial closet. Still not full floors. Digital is sealed. Fourth does not exist.
...
- Second P0 palette is registered (`floors/second-dimension/block-palette.md`) and generated into the Second dest climate only. Vestibule door 2 arrival is a material break into crimson mono (`second_wallpaper` / `second_carpet` / dest-side commercial door). TEMP atlas is Architect pack **v2** (lifted crimson). Debris fill is `second_debris_carpet`, never gray. No safelight on the mono body. Yellow-bleed is not registered this pass. No Second entities. **Regen dest / Second chunks** so the crimson atlas shows.
- Third P0 palette is registered (`floors/third-dimension/block-palette.md`) and generated into the Third dest climate only (`cold_garage`). Hard break from Second crimson: sodium fluorescent, wet concrete, lying stall paint, ramp-door liar mouth (opaque dark garage pane — not outdoor glass). Fill is `third_concrete`, never gray void. Chainlink / crate / headlights stay unregistered. No Third entities. No new live First / Second plane-cross — glance with `/brmc third`. **Regen Third chunks.**
...
- Dest stubs: regen dest chunks so return doors, Second crimson P0, and Third cold garage show. Mercy still fires in air at the identities if the old chunk has no block. Walk back through the matching dest-side threshold at the same XYZ. Do not use `/brmc second` / `/brmc third` (or other off-identity dest warps) to verify First returns. `/brmc third` is the Third dest glance: cold concrete / sodium, stall paint that does not map, ramp door that is a Door liar (not EXIT). No F3 pocket / dest / dim slugs (`brmc:first`) on public walks. Creative hover says Threshold / Opening, not vestibule_threshold / oob_hole.
